...
def receive_text_and_generate(message):
    ...
    try:
        with open(ref_path, "rb") as f:
            ...
            resp = requests.post(VOICE_API_URL, files=files, data=data, timeout=180)

        if resp.status_code != 200:
            logger.error(f"API error: {resp.status_code} {resp.text}")
            bot.edit_message_text(
                f"❌ បរាជ័យ: {resp.text[:200]}",
...
def handle_checkserver(message):
    try:
        resp = requests.get(VOICE_API_HEALTH, timeout=10)
        if resp.status_code == 200:
